...
def optimize_training_period(
    ...
    feature_cols: list,
    ticker: str = "AAPL",
    candidate_periods: tuple = ("1.5y", "2y", "3y"),
    train_ratio: float = 0.8,
    target_horizon: int = 20,
    verbose: bool = True,
):
    """
    複数の学習期間候補（1.5y, 2y, 3y 等）に対して時系列バックテスト評価を実施し、
    最も汎化性能（ROC-AUC, 適合率, 正解率）が高い最適学習期間を自動選定する
    """
    if verbose:
        print(f"\n[{ticker}] 学習期間の自動最適化 (Auto-Period Optimization) を実行中...")

    total_len = len(df_features)
    period_results = []

    for p in candidate_periods:
        target_days = PERIOD_DAYS.get(p, total_len)
        if target_days < total_len:
            sub_df = df_features.iloc[-target_days:].copy()
        else:
            sub_df = df_features.copy()

        sample_len = len(sub_df)
        # 最低サンプル数チェック（120日未満は除外）
        if sample_len < 120:
            if verbose:
                print(f"  ・候補 {p:<5} (データ: {sample_len:3d}日): サンプル数不足のためスキップ")
            continue

        # 同一行数の重複評価をスキップ（データが足りずに同じサイズになった場合）
        if any(r["sample_count"] == sample_len for r in period_results):
            continue
        ...
        # クオンツ評価スコア: ROC-AUC (40%), 適合率 (35%), 正解率 (25%)
        # 買い予測の誤爆(FP)を防ぎつつ、順序付け能力(AUC)を重視
        score = 0.40 * auc + 0.35 * prec + 0.25 * acc
        if auc < 0.50:
            score *= 0.5  # ランダム以下の逆相関モデルにはペナルティ
        ...
        if verbose:
            print(
                f"  ・候補 {p:<5} (学習対象: {sample_len:3d}日, テスト: {metrics['test_count']:3d}日): "
                f"ROC-AUC={auc:.4f}, 適合率={prec:.4f}, 正解率={acc * 100:.1f}%, F1={f1:.4f} "
                f"-> 汎化スコア={score:.4f}"
            )
    ...
    if not period_results:
        # フォールバック: 全データで学習
        model, metrics, best_thresh, df_imp = train_stock_model(
            df_features, feature_cols, ticker=ticker, train_ratio=train_ratio, target_horizon=target_horizon
        )
        return "all", model, metrics, best_thresh, df_imp, []

    # 最高スコアの候補を採用
    best_res = max(period_results, key=lambda x: x["score"])

    if verbose:
        print(f"  => 最適学習期間: 【{best_res['period']}】 を自動採用 (汎化スコア: {best_res['score']:.4f} [最高])")
