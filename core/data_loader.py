    ...
    try:
        ...
        if q_income is not None and not q_income.empty and len(q_income.columns) >= 4:
            ...
            for i, dt in enumerate(dates):
                ...
                if i >= 4:
                    prev_rev = (
                        q_income.loc["Total Revenue", dates[i - 4]] if "Total Revenue" in q_income.index else None
                    )
        ...
        # 2. 四半期データが不足している場合、年次決算推移（3〜4年分）から四半期推移を動的復元
            ...
            for dt in sorted(ann_income.columns):
                ...
                op_inc = (
                    float(ann_income.loc["Operating Income", dt]) if "Operating Income" in ann_income.index else None
                )
                diluted_shares = (
                    float(ann_income.loc["Diluted Average Shares", dt])
                    if "Diluted Average Shares" in ann_income.index
                    else None
                )
                eps = (
                    float(ann_income.loc["Diluted EPS", dt])
                    if "Diluted EPS" in ann_income.index
                    else (
                        net_inc / diluted_shares
                        if (net_inc and diluted_shares and diluted_shares > 0)
                        else trailing_eps
                    )
                )
                ann_records.append(
                    {"Date": pd.to_datetime(dt).normalize(), "Rev": rev, "NetInc": net_inc, "OpInc": op_inc, "EPS": eps}
                )
