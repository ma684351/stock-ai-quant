...
from core.sentiment import generate_news_dataset, analyze_sentiment_finbert
...
def analyze_single_stock(ticker: str, enable_deep_research: bool = False, force_refresh: bool = False, verbose: bool = True):
    """単一銘柄のエンドツーエンド分析を実行し、診断結果を返す"""
    ticker = ticker.upper().strip()
    if verbose:
        print("\n" + "=" * 70)
        print(f"【AI投資診断パイプライン実行開始: {ticker}】")
        print("=" * 70)
    ...
    if verbose:
        ...
        print("=" * 60)
        print(f"  ・{ticker} 直近終値          : ${latest_res['close']:.2f}")
        ...
        if latest_res['sentiment'] is not None:
            print(f"  ・ニュース感情スコア(FinBERT): {latest_res['sentiment']:+.3f}")
        print("  " + "-" * 56)
...
def print_comparison_table(results):
    ...
    print("=" * 92)
    header = f"{'順位':<4} {'銘柄':<8} {'現在株価':>10} {'動的PER':>9} {'14日RSI':>8} {'20日乖離':>9} {'1ヶ月上昇確率':>14}  {'判定結果':<16}"
    print(header)
    print("-" * 92)
    ...
    for i, r in enumerate(sorted_res):
        ...
        row = f"{i+1:2d}位  {r['ticker']:<8} ${r['close']:>9.2f} {pe_str:>9} {rsi_str:>8} {ma_str:>9} {prob_str:>14}  {decision:<16}"
        print(row)
    print("=" * 92 + "\n")

def main():
    parser = argparse.ArgumentParser(description="米国株AI投資判断パイプライン (FinBERT + LightGBM)")
    parser.add_argument("ticker", nargs="?", default=None, help="分析したいティッカーシンボル (例: AAPL, NVDA, TSLA)")
    parser.add_argument("--compare", nargs="+", help="複数銘柄を一括比較・ランキング (例: --compare AAPL NVDA GOOGL MSFT TSLA)")
    ...
    if args.compare:
        ...
        for t in tickers:
            try:
                ...
                results.append(res)
                print(f"  ✔ {t:<6} 完了 (現在値: ${res['close']:.2f}, 上昇確率: {res['prob']*100:.1f}%, 判定: {'買い' if res['is_buy'] else '見送り'})")
    ...
    # 3. 対話型モード
    print("\n" + "=" * 60)
    print("【米国株AI投資判断システム (FinBERT + LightGBM)】")
    print("=" * 60)
    user_input = input("分析したいティッカーシンボルを入力してください (例: AAPL, NVDA, TSLA または複数カンマ区切り): ").strip()
