...
```bash
# 日本株の自律リサーチ（東証適時開示・四半期決算・為替影響を調査）
python stock_ai.py 7203 --deep-research

# 米国株の自律リサーチ
python stock_ai.py TSLA --deep-research

# 既存キャッシュを無視して最新情報で再リサーチしたい場合
python stock_ai.py 7203 --deep-research --force
```
* **ローカルログインセッション自動連携**: ローカル環境（Antigravity CLI: `agy`）でログイン済みのセッションを自動認識し、**APIキー設定不要**で高速自律ディープリサーチを実行します。
* **日米バイリンガル対応**: 日本株の場合は日本語プロンプトで適時開示・上方修正・不祥事・為替影響等を調査し、米国株の場合はSEC開示・カンファレンスコール等を調査します。
* **高可用性フォールバック**:
...
* 調査された確定カタリストは `data/catalysts/{TICKER}.json` に自動保存され、2回目以降はキャッシュから高速ロードされてLightGBMと金融BERTの特徴量生成に即座に活用されます。
