...
過去の確定イベント・決算サプライズ・適時開示カタリストを自律調査させたい場合（`--deep-research` を指定するとキャッシュの有無に関わらず**問答無用でリサーチを実行**し最新データで上書きします）：
```bash
# 日本株の自律リサーチ（東証適時開示・四半期決算・為替影響などを調査）
python stock_ai.py 7203 --deep-research

# 米国株の自律リサーチ（SEC開示・決算カンファレンスコールなどを調査）
