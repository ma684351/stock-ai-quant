...
def mock_stock_data():
    ...
    volumes = np.concatenate([np.linspace(1000, 5000, 50), np.linspace(5000, 1000, 50)])
...
def test_calc_obv_features(mock_stock_data):
    ...
    expected_cols = ["AAPL_OBV_5d", "AAPL_OBV_20d", "AAPL_OBV_Signal_9d", "AAPL_OBV_Divergence"]
...
def test_calc_vroc_features(mock_stock_data):
    ...
    expected_cols = ["AAPL_VROC_5d", "AAPL_VROC_20d", "AAPL_VROC_MA3d"]
    for col in expected_cols:
        assert col in feats.columns
        assert not feats[col].isna().any()
...
def test_calc_bb_features(mock_stock_data):
    ...
    expected_cols = ["AAPL_BB_Upper", "AAPL_BB_Lower", "AAPL_BB_Width_Pct", "AAPL_BB_Position"]
    for col in expected_cols:
        assert col in feats.columns
        assert not feats[col].isna().any()
...
def test_calc_macd_features(mock_stock_data):
    ...
    expected_cols = ["AAPL_MACD", "AAPL_MACD_Signal", "AAPL_MACD_Histogram", "AAPL_MACD_Histogram_Cross"]
    for col in expected_cols:
        assert col in feats.columns
        assert not feats[col].isna().any()
